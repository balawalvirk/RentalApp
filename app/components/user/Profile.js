...
import { connectFirebase, saveData, getData, uploadImage } from '../../backend/firebase/utility';
...
export default class Profile extends Component {
  ...
  async getData() {
    ...
    await this.setState({
      ...
      imageUrl: {uri : userData['imageUrl']} ? {uri : userData['imageUrl']} : require('../../assets/avatar.png'),
  ...
  async onPress() {
    ...
    if(this.state.pictureUpload == true){
      alert('uploading')
      let name = currentUserId + ".jpg";
      let path = 'users/' + name;
      let docRef = {}
      docRef.id = currentUserId
      uploadImage(this.state.imageUrl.uri, 'image/jpeg', path, name, 'users', docRef, false)
    }
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps='handled'>
          ...
          <KeyboardAvoidingView behavior="padding" enabled>
            <View style={styles.boxSummary}>
              <TouchableOpacity onPress={()=>{this.selectImage()}} style={{ alignItems: 'center', marginTop: 5 }}>
